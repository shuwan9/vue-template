<template>
  <div class="supermarket-cart">
    <span>
      <i
        class="iconfont icongouwuche"
        @click="checkCart()"
        :class="supermarketCartTotal==0?'grey':''"
      >
        <button>{{supermarketCartTotal}}</button>
      </i>
    ...
    <span @click="goToOrder()" :class="supermarketCartTotal==0?'grey':''">确认下单</span>
    <supermarket-cart-detail></supermarket-cart-detail>
  </div>
</template>
...
import SupermarketCartDetail from "./SupermarketCartDetail";
...
export default {
  ...
  methods: {
    checkCart() {
      ...
      this.setShowSupermarketCartDetail(true);
    },
    ...mapMutations(["setShowSupermarketCartDetail"]),
    beforeDrop(el) {
    ...
    goToOrder() {
      ...
      this.$ls.set("spmorderBackPath", this.$route.path);
      this.$router.push("/spmconfirmorder");
    }
  },
  computed: {
